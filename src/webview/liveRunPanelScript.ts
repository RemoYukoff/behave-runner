...
(function () {
      ...
      function applyTreeRowBleed(
        el: HTMLElement,
        ancestorTreeChildDepth: number,
        bleedReducePx?: number
      ): void {
      ...
      /** Must match LIVE_PANEL_PROTOCOL_VERSION in src/ui/livePanelProtocol.ts */
      ...
      function setRowIcon(iconSpan, kind) {
        ...
        if (k === "pass") {
          ...
          iconSpan.className =
            base + " codicon codicon-loading codicon-modifier-spin";
        } else {
      ...
      function highlightMatchesInTextNode(textNode, escapedPattern) {
        ...
        var last = 0;
        /** @type {{ slice: string; mark: boolean }[]} */
        var chunks = [];
        var m;
      ...
      function applyConsoleFindHighlights(rawQuery) {
        ...
        if (
          !consoleOut.childNodes ||
          consoleOut.childNodes.length === 0 ||
          !q.length
        ) {
          consoleFindCounts.textContent = "";
          return;
      ...
      function openConsoleFind() {
        ...
        applyConsoleFindHighlights(
          consoleFindInput instanceof HTMLInputElement
            ? consoleFindInput.value
            : ""
        );
      ...
      function syncConsoleFindAfterRender() {
        ...
        var barOpen =
          !!consoleFindBar &&
          !consoleFindBar.classList.contains("console-find-bar--hidden");
        unwrapConsoleFindMarks();
        consoleFindMarks = [];
        consoleFindUiIndex = -1;
        if (!barOpen) {
        ...
        if (
          consoleFindInput instanceof HTMLInputElement &&
          String(consoleFindInput.value || "").trim().length > 0
        ) {
          applyConsoleFindHighlights(consoleFindInput.value);
        }
      }

      /** Shell visible — main area has tree + console. */
      function isLiveShellVisible() {
        return (
          !!shell &&
          !shell.classList.contains("hidden") &&
          !!emptyPlaceholder &&
          emptyPlaceholder.classList.contains("hidden")
        );
      }
      ...
      function findScenarioKeyForStepRow(row) {
        ...
        if (!det) return "";
        var sum = det.querySelector(":scope > summary") as HTMLElement | null;
      ...
      function findScenarioDetailsForKey(sk) {
        ...
        for (di = 0; di < icons.length; di++) {
          var sm = icons[di].querySelector(":scope > summary") as HTMLElement | null;
      ...
      /** Close scenarios still spinning after Behave finished (e.g. missed scenario_finished). */
      ...
      function handleLivePanelPayload(m) {
        ...
        if (m.type === "protocol") {
          livePanelProtocolOk =
            ...
            m.version === LIVE_PANEL_PROTOCOL_EXPECTED;
          if (!livePanelProtocolOk && emptyPlaceholder) {
            emptyPlaceholder.textContent =
              "Behave Runner: Live panel protocol mismatch. Reinstall or rebuild the extension.";
            emptyPlaceholder.classList.remove("hidden");
      ...
      document.addEventListener(
        ...
        function (ev) {
          ...
          if (
            ...
            consoleFindInput instanceof HTMLInputElement &&
            ev.key === "Enter" &&
            document.activeElement === consoleFindInput
          ) {
            ev.preventDefault();
            if (ev.shiftKey) findConsolePrevMark();
            else findConsoleNextMark();
